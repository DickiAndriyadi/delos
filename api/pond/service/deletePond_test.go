...
func Test_service_DeletePond(t *testing.T) {
	type args struct {
		id string
	}
	var (
	...
	tests := []struct {
...

		{
			...
			expectFunc: func(t *TestingService) {
				...
				t.pondRepository.On("DeletePond", &db, mock.Anything).Return(errors.New("internal server error"))
			},
			want: "",
			want1: &response.ErrorResponse{
				ErrCode: http.StatusInternalServerError,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, got1 := testing.Service.DeletePond(tt.args.id)
